...
import tn.ensit.miniprojetbibliotheque.ListeEmpruntController;
import tn.ensit.miniprojetbibliotheque.SidebarController;
...
public class MainController implements Initializable, BookReturnCallback {
    ...
    private void loadIssueOperation(ActionEvent event) {
        ...
        if (bookStatus.getText().equals(BOOK_NOT_AVAILABLE)) {
            ...
            AlertMaker.showMaterialDialog(rootPane, rootAnchorPane, Arrays.asList(btn, viewDetails), "Already issued book", "This book is already issued. Can't process issue request");
            return;
        }
        ...
        yesButton.addEventHandler(MouseEvent.MOUSE_CLICKED, (MouseEvent event1) -> {
            ...
            if (databaseHandler.execAction(str) && databaseHandler.execAction(str2)) {
                ...
                AlertMaker.showMaterialDialog(rootPane, rootAnchorPane, Arrays.asList((JFXButton) button), "Book Issue Complete", null);
                refreshGraphs();
            } else {
                Button button = new Button("Okay.I'll Check");
                AlertMaker.showMaterialDialog(rootPane, rootAnchorPane, Arrays.asList((JFXButton) button), "Issue Operation Failed", null);
            }
            clearIssueEntries();
        });
        ...
        noButton.addEventHandler(MouseEvent.MOUSE_CLICKED, (MouseEvent event1) -> {
            ...
            AlertMaker.showMaterialDialog(rootPane, rootAnchorPane, Arrays.asList((JFXButton) button), "Issue Cancelled", null);
            clearIssueEntries();
        });
    ...
    private void loadBookInfo2(ActionEvent event) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                Button button = new Button("Okay.I'll Check");
                AlertMaker.showMaterialDialog(rootPane, rootAnchorPane, Arrays.asList((JFXButton) button), "No such Book Exists in Issue Database", null);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private void loadSubmissionOp(ActionEvent event) {
        if (!isReadyForSubmission) {
            ...
            AlertMaker.showMaterialDialog(rootPane, rootAnchorPane, Arrays.asList((JFXButton) btn), "Please select a book to submit", "Cant simply submit a null book :-)");
            return;
        }
        ...
        yesButton.addEventHandler(MouseEvent.MOUSE_CLICKED, (MouseEvent ev) -> {
            ...
            if (databaseHandler.execAction(ac1) && databaseHandler.execAction(ac2)) {
                ...
                AlertMaker.showMaterialDialog(rootPane, rootAnchorPane, Arrays.asList((JFXButton) btn), "Book has been submitted", null);
                disableEnableControls(false);
                submissionDataContainer.setOpacity(0);
            } else {
                Button btn = new Button("Okay.I'll Check");
                AlertMaker.showMaterialDialog(rootPane, rootAnchorPane, Arrays.asList((JFXButton) btn), "Submission Has Been Failed", null);
            }
        });
        JFXButton noButton = new JFXButton("No, Cancel");
        noButton.addEventHandler(MouseEvent.MOUSE_CLICKED, (MouseEvent ev) -> {
            Button btn = new Button("Okay!");
            AlertMaker.showMaterialDialog(rootPane, rootAnchorPane, Arrays.asList((JFXButton) btn), "Submission Operation cancelled", null);
        });
    ...
    private void loadRenewOp(ActionEvent event) {
        if (!isReadyForSubmission) {
            Button btn = new Button("Okay!");
            AlertMaker.showMaterialDialog(rootPane, rootAnchorPane, Arrays.asList((JFXButton) btn), "Please select a book to renew", null);
            return;
        }
        JFXButton yesButton = new JFXButton("YES, Please");
        yesButton.addEventHandler(MouseEvent.MOUSE_CLICKED, (MouseEvent event1) -> {
            ...
            if (databaseHandler.execAction(ac)) {
                Button btn = new Button("Alright!");
                AlertMaker.showMaterialDialog(rootPane, rootAnchorPane, Arrays.asList((JFXButton) btn), "Book Has Been Renewed", null);
                disableEnableControls(false);
                submissionDataContainer.setOpacity(0);
            } else {
                Button btn = new Button("Okay!");
                AlertMaker.showMaterialDialog(rootPane, rootAnchorPane, Arrays.asList((JFXButton) btn), "Renew Has Been Failed", null);
            }
        });
        JFXButton noButton = new JFXButton("No, Don't!");
        noButton.addEventHandler(MouseEvent.MOUSE_CLICKED, (MouseEvent event1) -> {
            Button btn = new Button("Okay!");
            AlertMaker.showMaterialDialog(rootPane, rootAnchorPane, Arrays.asList((JFXButton) btn), "Renew Operation cancelled", null);
        });
    ...
    private void handleMenuAddMember(ActionEvent event) {
        LibraryAssistantUtil.loadWindow(getClass().getResource("member_add.fxml"), "Add New Member", null);
    }
    ...
    private void handleMenuViewMemberList(ActionEvent event) {
        LibraryAssistantUtil.loadWindow(getClass().getResource(""), "Member List", null);
    }
    ...
    private void handleIssuedList(ActionEvent event) {
        ...
        if (controller != null) {
            ListeEmpruntController cont = (ListeEmpruntController) controller;
